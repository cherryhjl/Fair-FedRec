...
import random
# from pylab import *
from fcf_ml100k_sign_1 import train_fcf_ml100k_sign
...
if __name__ == '__main__':
    ...
    valid_imp_len = []
    with open("./ml_100k/valid.data", 'r') as f:
        for l in f:
            uid, mid = l.strip('\n').split('\t')
            if int(uid) > last:
    ...
    test_imp_len = []
    with open("./ml_100k/test.data", 'r') as f:
        for l in f:
            uid, mid = l.strip('\n').split('\t')
            if int(uid) > last:
                neg = newsample(neg_samples[last], count * 4)
    ...
    ori_time_sign, path_sign = train_fcf_ml100k_sign(users_num, items_num, train_samples, neg_samples, valid_samples,
                     valid_imp_len, test_samples, test_imp_len)

    path_dir_all = ['fcf_ml100k']
    for path_dir in path_dir_all:
        total_ndcg, ndcg_var, last_var_exp = provider_fair_uni_qual(users_num, items_num, test_samples, ori_time_sign,
                                                                   test_imp_len, path_dir, 0.9,0.0015)
