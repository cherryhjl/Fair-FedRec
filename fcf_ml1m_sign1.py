...
def train_fcf_ml1m_sign(users_num, items_num, train_samples, neg_samples, valid_samples,
    ...
    path_dir = f'./fcf_ml1m_sign1/fcf_ml1m_sign1_local/model_{local_dir}'
    ...
    for r in range(200):
        ...
        for cnt, user in enumerate(users):
            ...
            if os.path.exists(f'./fcf_ml1m_sign1/fcf_ml1m_sign1_local/model_{local_dir}/{user_id}.pkl'):
                cp_train_para = torch.load(
                    f'./fcf_ml1m_sign1/fcf_ml1m_sign1_local/model_{local_dir}/{user_id}.pkl')
            else:
            ...
            torch.save(cp_model.state_dict(),
                       f'./fcf_ml1m_sign1/fcf_ml1m_sign1_local/model_{local_dir}/{user}.pkl')
        ...
        if (r + 1) % 200 == 0:
            ...
            for sample in valid_samples:
                ...
                u = sample[0]
                u_id = int(u[0])
                if os.path.exists(f'./fcf_ml1m_sign1/fcf_ml1m_sign1_local/model_{local_dir}/{u_id}.pkl'):
                    model_para = torch.load(
                        f'./fcf_ml1m_sign1/fcf_ml1m_sign1_local/model_{local_dir}/{u_id}.pkl')
                else:
            ...
            with open(f'./fcf_ml1m_sign1/fcf_ml1m_sign1_rslt/{name}-{f_txt}.txt', 'a') as f:
                f.write(
                    f"[{name}] :\n"
                    f"val [{time}] round auc: {val_auc:.4f}, mrr: {val_mrr:.4f}, ndcg5: {val_ndcg:.4f}, ndcg10: {val_ndcg10:.4f}\n")
            torch.save(agg.model.state_dict(), f'./fcf_ml1m_sign1/fcf_ml1m_sign1_model/{name}-{time}.pkl')
    ...
    for sample in test_samples:
        ...
        u_id = int(u[0])
        if os.path.exists(f'./fcf_ml1m_sign1/fcf_ml1m_sign1_local/model_{local_dir}/{u_id}.pkl'):
            model_para = torch.load(f'./fcf_ml1m_sign1/fcf_ml1m_sign1_local/model_{local_dir}/{u_id}.pkl')
        else:
            model_para = torch.load(f'./fcf_ml1m_sign1/fcf_ml1m_sign1_model/{name}-{time}.pkl')
        model.load_state_dict(model_para)
        with torch.no_grad():
    ...
    np.save(f'./fcf_ml1m_sign1/test_scores/{name}-{ori_time}.npy', test_preds)
    path = 'fcf_ml1m_sign1'
    ...
    test_auc_var, test_mrr_var, test_ndcg_var, test_ndcg10_var = 0, 0, 0, 0
